...
from group.abstractions.group_sign_abstraction import SignSet
...
class PerVarFrame[AV]:
    ...
    #widening
    def widen(self, other: "PerVarFrame") -> "PerVarFrame":
        new_locs = {k: SignSet.widen(self.locals[k], other.locals[k]) for k in self.locals}
        h = max(len(self.stack.items), len(other.stack.items))
        def get(s, i): return s.items[i] if i < len(s.items) else SignSet.empty()
        new_stack = Stack([SignSet.widen(get(self.stack, i), get(other.stack, i)) for i in range(h)])
        return PerVarFrame(new_locs, new_stack, self.pc)
...
class AState[AV]:
    ...
    #widening
    def widen(self, other: "AState[AV]") -> "AState[AV]":
        new_heap = {addr: AV.widen(self.heap.get(addr, AV.empty()), other.heap.get(addr, AV.empty()))
                    for addr in set(self.heap) | set(other.heap)}
        # For frames, do similar widening for locals & stack
        new_frames = []
        for f1, f2 in zip(self.frames.items, other.frames.items):
            new_frames.append(f1.widen(f2))
        # constraints: widen each variable's AV
        new_constraints = {k: AV.widen(self.constraints.get(k, AV.empty()),
                                       other.constraints.get(k, AV.empty()))
        for k in set(self.constraints) | set(other.constraints)}
        return AState(new_heap, Stack(new_frames), new_constraints)
...
#widening
class UnboundedAnalyzer:
    def __init__(self, domain, widen_threshold=3):
        self.domain = domain
        self.widen_threshold = widen_threshold

    def run(self, sts: StateSet) -> StateSet:
        update_count = defaultdict(int)  # Track how many times a PC has been joined

        while sts.needswork:
            pc = sts.needswork.pop()
            current_state = sts.per_inst.get(pc)
            if current_state is None:
                continue

            results = step(current_state, self.domain)

            for res in results:
                if not isinstance(res, AState):
                    # Terminal string outcome
                    sts.per_inst[pc] = res
                    continue

                new_pc = res.frames.peek().pc
                old_state = sts.per_inst.get(new_pc)

                if old_state is None:
                    sts.per_inst[new_pc] = res
                    sts.needswork.add(new_pc)
                    update_count[new_pc] = 1
                    continue

                # normal join
                joined = old_state.join(res)
                if joined is None:
                    joined = res

                update_count[new_pc] += 1
                count = update_count[new_pc]

                if count >= self.widen_threshold:
                    # apply widening
                    widened = old_state.widen(joined)
                    if widened != old_state:
                        sts.per_inst[new_pc] = widened
                        sts.needswork.add(new_pc)
                else:
                    # normal join
                    if joined != old_state:
                        sts.per_inst[new_pc] = joined
                        sts.needswork.add(new_pc)
...
all_ops = list(_suite.method_opcodes(method))  # all instructions in the method
...
for i in range(MAX_STEPS):
    for s in manystep(sts, SignSet):
        if isinstance(s, str):
            ...
            sts |= s
            # track visited PCs
            op_hit.update(s.frames.peek().pc for s in [s])

print(f"The following final states {final} are possible in {MAX_STEPS} steps")

# print instructions that were never reached
...
# -------------------- HYBRID ANALYSIS --------------------
def run_analysis_hybrid(sts, domain, max_steps=100, widen_threshold=3):
    update_count = defaultdict(int)  # track per-PC updates
    
    # Step 1: bounded analysis with update counts
    for _ in range(max_steps):
        for s in manystep(sts, domain):
            if isinstance(s, str):
                ...
                pc = s.frames.peek().pc
                old = sts.per_inst.get(pc)
                sts |= s
                if old != sts.per_inst[pc]:
                    update_count[pc] += 1

    # Step 2: find looping PCs (exceed threshold)
    looping_pcs = {pc for pc, count in update_count.items() if count >= widen_threshold}
    
    if looping_pcs:
        # Step 3: run unbounded widening only for looping PCs
        unbounded_analyzer = UnboundedAnalyzer(domain, widen_threshold=widen_threshold)
        subset_sts = StateSet(
            per_inst={pc: sts.per_inst[pc] for pc in looping_pcs},
            needswork=looping_pcs.copy()
        )
        sts_unbounded = unbounded_analyzer.run(subset_sts)
        
        # Step 4: update original states with unbounded results
        for pc, state in sts_unbounded.per_inst.items():
            sts.per_inst[pc] = state

    return sts



sts_hybrid = initialstate_from_method(method, SignSet)
final_hybrid = run_analysis_hybrid(sts_hybrid, SignSet, max_steps=10, widen_threshold=2)
print("Hybrid bounded + unbounded analysis done!")


# print("Analysis results per PC:")
# for pc, state in sts_hybrid.per_inst.items():
#     print(f"PC: {pc}")
#     print(state)
#     print("-" * 40)
